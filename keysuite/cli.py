...
def grammar_path() -> Path:
    candidates = [
        Path(__file__).resolve().parent / "data" / DEFAULT_GRAMMAR_FILE,
        Path(__file__).resolve().parents[2] / GRAMMAR_FILE,
...
def _trace_debug_lines(result: dict, debug_level: int) -> list[str]:
    ...
    if debug_level == 2:
        return [_trace_line(entry) for entry in result["trace"] if entry.get("type") == "transition"]
...
def _dispatch(ns: argparse.Namespace, grammar: dict, table: dict, grammar_file: Path, grammar_hash: str) -> int:
    ...
    if command in {"run", "reduce"}:
        ...
        session = RuntimeSession(
            grammar,
            table,
            config=RuntimeConfig.from_sources(explicit={"debug_level": active_debug}),
            auto_commit=not no_auto_commit,
            debug_level=active_debug,
        )
        result = session.process(tokens, finalize=True)
...
def _build_runtime_config(ns: argparse.Namespace) -> RuntimeConfig:
    explicit: dict[str, object] = {}
    if getattr(ns, "grammar", None):
        explicit["grammar_path"] = getattr(ns, "grammar")
    if getattr(ns, "debug", None) is not None:
        explicit["debug_level"] = getattr(ns, "debug")
    return RuntimeConfig.from_sources(explicit=explicit, env=os.environ)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        config = _build_runtime_config(ns)
        explicit_grammar = getattr(ns, "grammar", None)
        if explicit_grammar:
            grammar_file = Path(explicit_grammar)
        else:
            configured = Path(config.grammar_path) if config.grammar_path else None
            grammar_file = configured if configured and configured.is_file() else grammar_path()
        grammar, table, grammar_hash = _load_runtime(grammar_file)
