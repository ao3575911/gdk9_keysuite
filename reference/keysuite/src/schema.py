...
def validate_grammar(grammar: dict) -> dict:
    ...
    if _require_string(grammar.get("version"), "version") != EXPECTED_VERSION:
        raise ValueError(f"unsupported grammar version: {grammar.get('version')}")
    if _require_string(grammar.get("standard_version"), "standard_version") != EXPECTED_VERSION:
        raise ValueError(f"unsupported standard version: {grammar.get('standard_version')}")
    if (
